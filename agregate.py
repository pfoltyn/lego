...
for id, value in lego.iteritems():
    vlen0 = min(len(value[0].keys()), 1)
    vlen1 = min(len(value[1].keys()), 1)
    ...
    for x in range(min(vlen0, vlen1)):
        if x == 0:
            print "{},{},={}*$H$1,\"=INDIRECT(ADDRESS(ROW(), COLUMN()-2, 4))-INDIRECT(ADDRESS(ROW(), COLUMN()-1, 4))\",{},{}".format(id, value[0].keys()[x], value[1].keys()[x], value[0][value[0].keys()[x]], value[1][value[1].keys()[x]])
        else:
